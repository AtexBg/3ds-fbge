...
void executeCommand(const char *cmd) { //when i wrote that code, only me and God understood it, now only God does. (AtexBg, 28/01/26)
    FILE *fp;
    char result[256];
    char cmdLower[256];

    //convert command to lowercase so the shell can use it 
    for (size_t i = 0; cmd[i] != '\0' && i < sizeof(cmdLower) - 1; i++) {
    ...
    pclose(fp); //close fp, but i don't f*cking remember what "fp" is (edit: maybe it's some pointer to data?)

    //disp. cmd + result
    snprintf(termBuffer, TERM_BUF_SIZE, "#~ %.2000s\n%.2000s#~ ", cmd, cmdOut); //compiler warning here (idek?) ((Update: fixed with "%.2000s"))
    termBuffer[TERM_BUF_SIZE - 1] = '\0';
}
...
void handleKeyboard() {
    ...
    while (read(STDIN_FILENO, &c, 1) > 0) {
        ...
        if(len >= TERM_MAX_COLS){
            strcat(termBuffer, "\n");
            len = 0; //reset length of buffer
        }
        ...
        //termBuffer[len + 1] = '_'; //maybe i should uncomment that
    }
}
